"use strict";
// @generated by protoc-gen-es v0.2.1 with parameter "target=ts"
// @generated from file status/v1/status.proto (package status.v1, syntax proto3)
/* eslint-disable */
/* @ts-nocheck */
var __extends = (this && this.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (Object.prototype.hasOwnProperty.call(b, p)) d[p] = b[p]; };
        return extendStatics(d, b);
    };
    return function (d, b) {
        if (typeof b !== "function" && b !== null)
            throw new TypeError("Class extends value " + String(b) + " is not a constructor or null");
        extendStatics(d, b);
        function __() { this.constructor = d; }
        d.prototype = b === null ? Object.create(b) : (__.prototype = b.prototype, new __());
    };
})();
Object.defineProperty(exports, "__esModule", { value: true });
exports.StatusServiceGetResponse = exports.StatusServiceGetRequest = void 0;
var protobuf_1 = require("@bufbuild/protobuf");
var health_pb_js_1 = require("../../api/v1/health_pb.js");
/**
 * @generated from message status.v1.StatusServiceGetRequest
 */
var StatusServiceGetRequest = /** @class */ (function (_super) {
    __extends(StatusServiceGetRequest, _super);
    function StatusServiceGetRequest(data) {
        var _this = _super.call(this) || this;
        protobuf_1.proto3.util.initPartial(data, _this);
        return _this;
    }
    StatusServiceGetRequest.fromBinary = function (bytes, options) {
        return new StatusServiceGetRequest().fromBinary(bytes, options);
    };
    StatusServiceGetRequest.fromJson = function (jsonValue, options) {
        return new StatusServiceGetRequest().fromJson(jsonValue, options);
    };
    StatusServiceGetRequest.fromJsonString = function (jsonString, options) {
        return new StatusServiceGetRequest().fromJsonString(jsonString, options);
    };
    StatusServiceGetRequest.equals = function (a, b) {
        return protobuf_1.proto3.util.equals(StatusServiceGetRequest, a, b);
    };
    StatusServiceGetRequest.runtime = protobuf_1.proto3;
    StatusServiceGetRequest.typeName = "status.v1.StatusServiceGetRequest";
    StatusServiceGetRequest.fields = protobuf_1.proto3.util.newFieldList(function () { return []; });
    return StatusServiceGetRequest;
}(protobuf_1.Message));
exports.StatusServiceGetRequest = StatusServiceGetRequest;
/**
 * @generated from message status.v1.StatusServiceGetResponse
 */
var StatusServiceGetResponse = /** @class */ (function (_super) {
    __extends(StatusServiceGetResponse, _super);
    function StatusServiceGetResponse(data) {
        var _this = _super.call(this) || this;
        /**
         * @generated from field: api.v1.ServiceStatus overall_status = 2;
         */
        _this.overallStatus = health_pb_js_1.ServiceStatus.UNSPECIFIED;
        /**
         * @generated from field: bool connection_broken = 3;
         */
        _this.connectionBroken = false;
        /**
         * @generated from field: string api_version = 4;
         */
        _this.apiVersion = "";
        protobuf_1.proto3.util.initPartial(data, _this);
        return _this;
    }
    StatusServiceGetResponse.fromBinary = function (bytes, options) {
        return new StatusServiceGetResponse().fromBinary(bytes, options);
    };
    StatusServiceGetResponse.fromJson = function (jsonValue, options) {
        return new StatusServiceGetResponse().fromJson(jsonValue, options);
    };
    StatusServiceGetResponse.fromJsonString = function (jsonString, options) {
        return new StatusServiceGetResponse().fromJsonString(jsonString, options);
    };
    StatusServiceGetResponse.equals = function (a, b) {
        return protobuf_1.proto3.util.equals(StatusServiceGetResponse, a, b);
    };
    StatusServiceGetResponse.runtime = protobuf_1.proto3;
    StatusServiceGetResponse.typeName = "status.v1.StatusServiceGetResponse";
    StatusServiceGetResponse.fields = protobuf_1.proto3.util.newFieldList(function () { return [
        { no: 1, name: "health", kind: "message", T: health_pb_js_1.Health },
        { no: 2, name: "overall_status", kind: "enum", T: protobuf_1.proto3.getEnumType(health_pb_js_1.ServiceStatus) },
        { no: 3, name: "connection_broken", kind: "scalar", T: 8 /* ScalarType.BOOL */ },
        { no: 4, name: "api_version", kind: "scalar", T: 9 /* ScalarType.STRING */ },
    ]; });
    return StatusServiceGetResponse;
}(protobuf_1.Message));
exports.StatusServiceGetResponse = StatusServiceGetResponse;
